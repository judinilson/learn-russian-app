...
export class QuestionsAnswersComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    else this._start = !this._start;

    this.trainingTestService.currrentest.subscribe(
    ...
    this.totalQestions = this.testData.trainings.length;
  ...
  onConfirm(): void {
    if (this.confirmCount == 1) {
      ...
      if (!this.answerform.valid) {
        ...
        console.log(
          `total correct Answer:${this.totalCorrectAnswer}  total incorrect Answer: ${this.totalIncorrectAnswer}`
        );
        return;
      }
      // }
      ...
      // if (
      //   this.dragDropDataContainer === undefined &&
      //   (this.isDragDropQuestionfirst || this.isDragDropQuestionsecond)
      // ) {
      //   this.dismissConfirmAlert("please complete the task by chosing answer.");
      //   this.confirmCount = 1;
      //   return;
  ...
  choseQuetions(changeQuetionindex) {
    ...
    if (this._start === true) {
      ...
      var data = new Array(this.testData.trainings.length);
    ...
    if (this.randomIndexs.length !== 0) {
      console.log("choseen index: ", this.randomIndexs[changeQuetionindex]);
      console.log("test data: ", this.testData.trainings);
      var choseenIndex = this.randomIndexs[changeQuetionindex];
      this.currentQuestion = this.testData.trainings[choseenIndex].questions;
      this.currentAnswers = this.testData.trainings[choseenIndex].answers;
  ...
  _countbarPercent(index) {
    ...
    else this.countPercent = (100 * index) / +this.totalQestions;
  }
  ...
  testIsDone() {
    var rest =
      this.totalAnswers - (this.totalCorrectAnswer + this.totalIncorrectAnswer);
  ...
  storeResult(totalAnswers) {
    ...
    localStorage.setItem(
      "trainingDataResult",
      JSON.stringify(_trainingDataResult)
    );
  }
  ...
  createStatistic(totalAnswers) {
    var correctAnswerPercentage = Math.round(
      (this.totalCorrectAnswer * 100) / totalAnswers
    );
    var incorrectAnswerPercentage = Math.round(
      (this.totalIncorrectAnswer * 100) / totalAnswers
    );
