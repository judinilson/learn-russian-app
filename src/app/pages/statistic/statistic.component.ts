...
export class StatisticComponent implements OnInit {
    ...
    if (this.trainingDataResult !== null) {
      ...
      (this.totalCorrectAnswers = this.trainingDataResult.totalCorrectAnswers),
        (this.totalIncorrectAnswers =
          this.trainingDataResult.totalIncorrectAnswers),
  ...
  _calculatePercent() {
    this.correctAnswerPercentage = Math.round(
      (this.totalCorrectAnswers * 100) / this.totalAnswers
    );
    this.incorrectAnswerPercentage = Math.round(
      (this.totalIncorrectAnswers * 100) / this.totalAnswers
    );
  ...
  calculateRating(correctAnswerPercentage) {
    if (correctAnswerPercentage >= 0 && correctAnswerPercentage <= 60)
      this.rating = 1;
    else if (correctAnswerPercentage > 60 && correctAnswerPercentage <= 70)
      this.rating = 2;
    else if (correctAnswerPercentage > 70 && correctAnswerPercentage <= 80)
      this.rating = 3;
    else if (correctAnswerPercentage > 80 && correctAnswerPercentage <= 90)
      this.rating = 4;
    else this.rating = 5;
  }
