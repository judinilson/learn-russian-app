...
export class AdminDashboardComponent implements OnInit {
    ...
    dialogRef
      ...
      .subscribe((result) => {
        ...
        if (result != null) {
          var data = result;
          this.contentService
            .updateCategory(data.id, { name: data.name })
            .pipe(debounceTime(300))
            .pipe(distinctUntilChanged())
            .subscribe(
              (response) => {
    ...
    this.identityService
      ...
      .subscribe(
        (data) => {
          ...
          created.forEach((c) => {
            if (
              c.getDate() == this.lastWeek.getDate() ||
              c.getDate() == this.today.getDate()
            ) {
      ...
      .subscribe(
        (data) => {
          ...
          statisticsDate.forEach((s, i) => {
            if (
              s.getDate() == this.lastWeek.getDate() ||
              s.getDate() == this.today.getDate()
            ) {
  ...
  lastWeekStatisticsPercentages(lastWeekUsersStatistics) {
    ...
    st.forEach((s, i) => {
      ...
      if (s.date.getDay() === 3) {
        //wednesday
        incorrectPercentagewednesday =
          s.ipercentage + incorrectPercentagewednesday;
      ...
      if (s.date.getDay() === 4) {
        //thursday
        incorrectPercentagethursday =
          s.ipercentage + incorrectPercentagethursday;
      ...
      if (s.date.getDay() === 6) {
        //saturday
        incorrectPercentagesaturday =
          s.ipercentage + incorrectPercentagesaturday;
        correctPercentagesaturday = s.cpercentage + correctPercentagesaturday;
        saturdayCount += 1;
      }
    });

    this.barChartData[0].data[0] = Math.round(
      incorrectPercentagemoday / mondayCount
    );
    this.barChartData[1].data[0] = Math.round(
      correctPercentagemonday / mondayCount
    );

    this.barChartData[0].data[1] = Math.round(
      incorrectPercentagetuesday / tuesdayCount
    );
    this.barChartData[1].data[1] = Math.round(
      correctPercentagetuesday / tuesdayCount
    );

    this.barChartData[0].data[2] = Math.round(
      incorrectPercentagewednesday / wednesdayCount
    );
    this.barChartData[1].data[2] = Math.round(
      correctPercentagewednesday / wednesdayCount
    );

    this.barChartData[0].data[3] = Math.round(
      incorrectPercentagethursday / thursdayCount
    );
    this.barChartData[1].data[3] = Math.round(
      correctPercentagethursday / thursdayCount
    );

    this.barChartData[0].data[4] = Math.round(
      incorrectPercentagefriday / fridayCount
    );
    this.barChartData[1].data[4] = Math.round(
      correctPercentagefriday / fridayCount
    );

    this.barChartData[0].data[5] = Math.round(
      incorrectPercentagesaturday / saturdayCount
    );
    this.barChartData[1].data[5] = Math.round(
      correctPercentagesaturday / saturdayCount
    );
  }
