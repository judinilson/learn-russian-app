...
export class UsersManagementComponent implements OnInit {
  ...
  getAllGroupRelatedToTeacher() {
    this.identityService
      ...
      .subscribe(
        ...
        (error) => {
          console.log(
            "Ocurred error trying to get releted teachers group: ",
            error
          );
        }
      );
        ...
        this.studentUsers.forEach((x) => {
          var ct =
            this.countries !== undefined
              ? this.countries.filter((c) => c.id == x.countryId)
              : []; //country
          var tg =
            this.teacherGroups !== undefined
              ? this.teacherGroups.filter((t) => t.id == x.teacherGroupId)
              : []; //teacher group(the relation between teacher and group)
          var teacher =
            this.Users !== undefined
              ? this.Users.filter((u) => u.id == tg[0].teacherId)
              : []; // teacher
          var g =
            this.groups !== undefined
              ? this.groups.filter((g) => g.id == tg[0].groupId)
              : []; //group
          var st =
            this.statistics !== undefined
              ? this.statistics.filter((s) => s.userId == x.id)
              : []; //statistic
  ...
  */
  ...
  userStudentCreateDialog() {
    var groups = [];
    this.groupAndTeacher.forEach((x) => {
      groups.push(x.group);
    });
      ...
      .subscribe((result) => {
        ...
        if (result != null) {
          if (result.groupId === 0) {
            ...
            if (this.teacherGroups != undefined) {
              reletedTeacherGroup = this.teacherGroups.filter(
                (tg) => tg.groupId == result.groupId
              ); //filter the groups tha have teachers releted to that group id
  ...
  */
  ...
  createTeacherGroup(groupId, teacherId, time) {
    this.identityService
      ...
      .subscribe(
        ...
        (error) => {
          this.identityService.deleteGroup(groupId).subscribe();
          this.alertService.openSweetAlertToast("error", error);
          console.log(error);
        }
      );
  ...
  collapse(type: string) {
    if (type === "student")
      this.collapsetableStudent = !this.collapsetableStudent;
    if (type === "statistic")
      this.collapsetableStudentStatistic = !this.collapsetableStudentStatistic;
    if (type === "teacher")
      this.collapsetableTeacher = !this.collapsetableTeacher;
