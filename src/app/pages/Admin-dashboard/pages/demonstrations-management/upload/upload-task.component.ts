...
export class UploadTaskComponent implements OnInit {
  ...
  uploadData() {
          ...
          this.contentService.uploadCoverImage(
            ...
            .subscribe(
              res => {
                //back to table 
                imgurl = res
                if (res != null) this.uploadMediaFilesAndContent(this.demoContentsrc, imgurl)
    ...
    if (this.isUpdate) {
      if (!this.uploadForm.invalid) {
        ...
        var catg = this._uploadForm.category.value.id === undefined || this._uploadForm.category.value.id === this.selectedcategory[0].id
          ? this.receiveSelectedData.category.id : this._uploadForm.category.value.id
  ...
  updateContentFunc(imageUpdate, catg) {
    ...
    this.contentService.updateDemoContent(this.receiveSelectedData.id, {
  ...
  uploadMediaFilesAndContent(_demoContentsrc, imgurl?) {
    ...
    var observable = new Observable(observer => {
      ...
      this.contentService.uploadSrcFiles(
        formData
      ).pipe(debounceTime(500))
        .pipe(distinctUntilChanged())
        .subscribe(
          res => {
            console.log("DATA source file Successfully uploaded: ", urldbmediafiles)
            if (res !== null) {
              mediafilesUrl = res
              ...
              */
              if (urldbmediafiles.length !== 0 && !this.isUpdate) {
                ...
                setTimeout(() => {
                  ...
                  this.contentService.uploadDemoSrc({
                    ...
                    .subscribe(
                      res => {
                        //back to table 
                        console.log(" Demosource Successfully uploaded : ", res);
                        //if (!this.isUpdate) {
                        ...
                        setTimeout(() => {
                          ...
                          observer.next(2)
                          this.contentService.createDemoContent(
                            {
                              title: this._uploadForm.title.value,
                              subtitle: this._uploadForm.description.value,
              ...
              if (this.isUpdate) {
                if (this.selectedDemoSourceIDFromReceivedData !== undefined) {
                  observer.next(1)
                  this.contentService.updateDemoSrc(this.selectedDemoSourceIDFromReceivedData, urldbmediafiles
                  ).pipe(debounceTime(500))
                    .pipe(distinctUntilChanged())
                    .subscribe(
                      res => {
  ...
  deleteSource(source) {
    Swal.fire({
      ...
      if (result.value) {
        this.contentService.deletesource(source.id)
          .pipe(debounceTime(500))
          .pipe(distinctUntilChanged())
          .subscribe(
            data => {
      ...
      if (result.value) {
        ...
        if (this.demoContentsrc.length !== 0) {
          this.demoContentsrc.forEach((el, index) => {
            this.contentService.deletesource(el.id)
              .pipe(debounceTime(500))
              .pipe(distinctUntilChanged())
              .subscribe(
                data => {
        ...
        //DELETE DEMOSOURCE 
        this.contentService.deleteDemoSource(demosourceId)
          .pipe(debounceTime(500))
          .pipe(distinctUntilChanged())
          .subscribe(
            data => {
